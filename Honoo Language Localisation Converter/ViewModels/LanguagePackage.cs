...
namespace HonooLanguageLocalisationConverter.ViewModels
{
    ...
    public sealed class LanguagePackage
    {
        ...
        /// </summary>
        public string Creator { get; } = "Honoo Language Localisation Converter 1.0.5";

        /// <summary>
        /// Creator website.
        /// </summary>
        public string Website { get; } = "https://github.com/LokiHonoo/Honoo-Language-Localisation-Converter";
        ...
        /// </summary>
        /// <param name="defaultFields">Select current fields or default fields.</param>
        /// <param name="fileName">Language file name.</param>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0063:Use simple 'using' statement", Justification = "<Pending>")]
        public void Save(bool defaultFields, string fileName)
        {
            using (var manager = new XConfigManager())
            {
                ...
                this.ToastMessages.SaveInternal(defaultFields, manager);
        ...
        /// <param name="stream">Language stream.</param>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0063:Use simple 'using' statement", Justification = "<Pending>")]
        public void Save(bool defaultFields, Stream stream)
        {
            using (var manager = new XConfigManager())
            {
                manager.Default.Properties.AddString("Creator", this.Creator);
        ...
        public sealed class __Informartion : INotifyPropertyChanging, INotifyPropertyChanged
        {
            ...
            internal void LoadInternal(XConfigManager manager)
            {
                if (manager.Sections.TryGetValue("Informartion", out XSection section))
                {
                    ...
                    this.LangName = section.Properties.GetStringValue("LangName", _langName_d);
                    this.LangVer = section.Properties.GetStringValue("LangVer", _langVer_d);
                    this.Author = section.Properties.GetStringValue("Author", _author_d);
                    this.Email = section.Properties.GetStringValue("Email", _email_d);
                    this.Website = section.Properties.GetStringValue("Website", _website_d);
                    this.Remarks = section.Properties.GetStringValue("Remarks", _remarks_d);
                }
            }
